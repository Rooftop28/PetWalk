...
# 📋 功能全览 (Feature Overview)
*(Updated: 2026/04/07)*

## 一、首页（陪伴 Tab）
1. **宠物抠图展示** — Vision 框架自动抠图，透明背景贴图
2. **宠物心情系统** — 基于上次遛狗时间，4 种心情状态 + 动画/文案/贴纸
3. **用户形象** — Ready Player Me SDK 3D 形象创建
4. **骨头币显示** — 实时余额，点击进入奖励商店
5. **开始/结束遛狗** — GPS 全屏地图 + 实时轨迹 + 数据面板
6. **实时直播（云遛狗）** — 好友通过房间号观看遛狗实况并点赞
7. **Live Activity（灵动岛）** — 锁屏/灵动岛显示距离和时长
8. **DEBUG 工具** — 模拟心情、天气条件测试

## 二、足迹 Tab（历史记录）
9. **日历卡片** — 照片/热力图/AI 日记三模式切换
10. **统计面板** — 总里程、总时长汇总，可查看详情
11. **历史记录列表** — 按时间倒序展示
12. **遛狗详情页** — 静态轨迹、照片、详细数据
13. **AI 日记** — DeepSeek AI 基于宠物档案自动生成第一人称日记
14. **设置入口** — 右上角齿轮进入设置页

## 三、成就 Tab
15. **成就系统（PawPrints）** — 43 个成就，7 大类别（里程/频率/连续打卡/景点/速度/天气/特殊）
16. **隐藏成就** — ~44% 为隐藏，毛玻璃蒙层效果
17. **成就稀有度** — 普通/稀有/史诗/传说
18. **成就详情页** — 图标、描述、进度、稀有度、奖励
19. **排行榜** — 全球/同城/好友三范围（Game Center + Supabase）

## 四、奖励商店
20. **称号系统** — 6 个可购买称号，可装备展示
21. **主题配色** — 6 套主题，购买后即时切换
22. **成就线索商店** — 花骨头币揭示隐藏成就（随机 30 币/指定 50 币），3D 翻牌动画

## 五、遛狗结算
23. **成绩卡片** — 距离、时长
24. **骨头币 + 成就奖励** — 自动计算收益和解锁
25. **心情打分** — 给狗狗打分
26. **拍照/相册** — 支持摄像头拍照和相册选择
27. **AI 日记生成** — 基于遛狗数据和宠物性格生成

## 六、通知与社交
28. **每日遛狗提醒** — 多个提醒时间（最多 8 个），10 条随机文案
29. **好友催促** — 排行榜好友榜「催一下」，每天每人限一次

## 七、设置
30. **通知管理** — 提醒时间增删与开关
31. **账号管理** — Supabase 匿名认证，跨设备恢复
32. **编辑档案** — 修改宠物/主人信息
33. **宠物档案重填** — 重走性格/声音设置流程
34. **叫声录制** — 录制宠物叫声，可上传 Supabase
35. **数据同步** — iCloud/Supabase 云同步
...
## 九、底层能力
40. **GPS 定位与轨迹** — CoreLocation 后台追踪
41. **天气感知** — QWeather API（上线前切换 WeatherKit）
42. **POI 检测** — MapKit 搜索附近餐厅
43. **景点打卡** — 本地景点坐标库
44. **HealthKit** — 每日步数和步行距离
45. **Apple Game Center** — 认证、排行榜、成就同步
46. **Apple Watch 联动** — 传输宠物抠图到手表端显示

---

<a id="-v1-feature-flags"></a>
# 🏷️ V1 上线精简方案 (Feature Flags)
*(Updated: 2026/04/07)*

为尽快上线 App Store，V1 通过 `FeatureFlags.swift` 禁用以下功能（代码保留，开关即可恢复）：

| 功能 | Flag | 状态 | 原因 |
|------|------|------|------|
| Ready Player Me 用户形象 | `enableAvatar` | ❌ 禁用 | WebView 加载慢，依赖外部 SDK |
| 实时直播 / 云遛狗 | `enableLiveWalk` | ❌ 禁用 | 依赖 Supabase Realtime，复杂度高 |
| 好友催促推送 | `enableFriendNudge` | ❌ 禁用 | 需要远程推送后端，目前仅模拟 |
| 叫声录制 / 上传 | `enableVoiceRecording` | ❌ 禁用 | 依赖 Supabase Storage |
| 排行榜 | `enableLeaderboard` | ❌ 禁用 | 依赖 Supabase + Game Center |
| 云同步 (Supabase) | `enableCloudSync` | ❌ 禁用 | 第一版用本地数据 |
| Game Center 集成 | `enableGameCenter` | ❌ 禁用 | 第一版不接入 |
| 称号系统 | `enableTitleSystem` | ❌ 禁用 | 可后续加入 |

### V1 保留的核心功能
- 宠物抠图 + 心情系统
- GPS 遛狗 + 实时轨迹 + Live Activity
- 成就系统（43 个成就 + 隐藏成就 + 线索商店）
- 主题配色商店 + 骨头币经济
- 日历 + 统计 + AI 日记
- 每日遛狗提醒（多时间段）
- 拍照留念（相机 + 相册）
...
### 影响的文件
- `Core/Config/FeatureFlags.swift` — 功能开关定义
- `App/PetWalkApp.swift` — Game Center 认证
- `Core/Services/AppInitializer.swift` — Supabase 认证 + 云同步
- `Features/Home/Views/HomeView.swift` — 用户形象 + 云遛狗
- `Features/Achievement/Views/AchievementView.swift` — 排行榜入口
- `Features/Achievement/Views/RewardShopView.swift` — 称号 Tab
- `Features/Settings/Views/ReminderSettingsView.swift` — 账号/录音/同步
- `Features/Walk/Views/WalkSummaryView.swift` — 遛狗后云同步
